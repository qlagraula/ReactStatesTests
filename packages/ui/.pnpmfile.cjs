...
function readPackage(pkg, context) {
  ...
  if (pkg.name === '@box-extractor/vanilla-extract') {
    context.log(
      '[@box-extractor/vanilla-extract]: Replacing link: with git: dependencies',
    );
  ...
  if (
    pkg.name === '@vanilla-extract/vite-plugin' ||
    pkg.name === '@vanilla-extract/esbuild-plugin'
  ) {
    context.log(
      `[${pkg.name}]: Replacing @vanilla-extract/integration with local version`,
    );

    pkg.dependencies['@vanilla-extract/integration'] =
      baseUrl + 'integration.tgz';
  }
